...
The deadline for Homework 11 is Wednesday, December 8, 4pm. The late submission deadline is Monday, December 13, 4pm.

Refer to the homework handout [`hw11.pdf`](hw11.pdf) for details about the assignment. This file provides some information to help you get started with setting up your development environment for the homework assignments.
...
available at
`https://github.com/nyu-popl-fa21/hw11-<YOUR-GITHUB-USERNAME>`.
Note that this may take a few minutes.

* Open a browser at `https://github.com/nyu-popl-fa21/hw11-<YOUR-GITHUB-USERNAME>` with your Github username inserted at the appropriate place in the URL.
* Choose a place on your computer for your homework assignments to reside and open a terminal to that location.
* Execute the following git command: <br/>
  ```bash
  git clone https://github.com/nyu-popl-fa21/hw11-<YOUR-GITHUB-USERNAME>.git hw11
  cd hw11
  ```
...
The problems that you you should solve for this assignment are described in the file `hw11.pdf`.
...
2. The code template for solving Problem 2 is provided in the file 
   ```
   src/main/scala/popl/hw11.scala 
   ``` 
   ...
   expressions in `hw11.scala` with your code for the corresponding
...
The parsed expression is then passed to your implementation of the interpreter (the function `eval` defined in `hw11.scala`). For instance, to run your interpreter on the file `testjs/test01_arith.js`, you can execute the following command from the command line:
...
```
sbt:hw11> run testjs/test01_arith.js
```
...
Alternatively, you can run the interpreter directly from inside the IDE. To do so, right-click the file `hw11.scala` and select "More Run/Debug" and then "Modify Run Configuration". You can provide the command line arguments to the interpreter in the text field labeled "Program arguments". Enter e.g. "testjs/test01_arith.js" in this field and click "OK". When you now right-click on the file `hw11.scala` and select "Run 'hw11'", then the interpreter will be executed with the specified command line arguments.
